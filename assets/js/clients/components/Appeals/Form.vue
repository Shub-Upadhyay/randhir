					...
					>
						<b-form-group label="Level" label-for="appeal_level_id" label-cols-lg="4">
							<b-form-select
								...
								:options="availableAppealLevels"
								:disabled="saving || !availableAppealLevels || availableAppealLevels.length <= 0"
								:state="getValidationState(validationContext)"
	...
	methods: {
		...
		async autoFillForm() {
			try{
				...
				this.letterDate = this.formatDate(responses.data[0].letter_date);  
