...
class UserMailer extends Mailer
{
	...
	public function registrationSuccess(User $user): Mailer
    {
        return $this
            ->setTo($user->email, $user->full_name)
            ->setSubject(sprintf('Registration Success at %s', Configure::readOrFail('App.name')))
            ->setViewVars(compact('user'))
            ->setEmailFormat('html')
			->setTemplate('welcome_user'); // Set the email template layout to 'welcome_user.php'
        	// ->send(); // Send the email
    }
}
